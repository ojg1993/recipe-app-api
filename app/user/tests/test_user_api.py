...
class PublicUserApiTests(TestCase):
    ...
    def test_password_too_short(self):
        # Test error if password less than 5 chars
        payload = {
            'email': 'test@example.com',
            'password': 'qw',
            'name': 'Test name'
    ...
    def test_create_token_bad_credentials(self):
        # Test returning error with invalid credentials
    ...
    def test_create_token_blank_password(self):
        # Test returning error with a blank password
        payload = {
            'email': 'test@example.com',
            'password': '',
        }
...
class PrivateUserApiTests(TestCase):
    ...
    def test_post_me_not_allowed(self):
        # Test POST not allowed on the 'me' endpoint
        res = self.client.post(ME_URL, {})
        self.assertEqual(res.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)

    def test_update_user_profile(self):
        # Test updating the user profile for authenticated user
        payload = {'name': 'new name', 'password': 'testpassword123'}
